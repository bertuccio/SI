CREATE OR REPLACE FUNCTION tgCheckCorreccion() RETURNS TRIGGER AS $tgCheckCorreccion$
  DECLARE
    ...
    paginaRow pagina%ROWTYPE;
    correccionRow correccion%ROWTYPE;
  ...
  BEGIN
    IF OLD.fecha_fin IS NULL AND NEW.fecha_fin IS NOT NULL  THEN
	  ...
	  SELECT INTO paginaRow * FROM pagina WHERE n_pagina=32 AND isbn='0000000000541';
	  RAISE NOTICE 'La pagina 32 ha cambiado a la fase %.', paginaRow.id_fase;
	  SELECT INTO correccionRow * FROM correccion WHERE isbn='0000000000541' AND n_pagina=32 AND id_fase='TF';
	  RAISE NOTICE 'La fecha fin de la correccion de la pagina 32 en TF a cambiado a %.', correccionRow.fecha_fin;
	  RAISE NOTICE 'El numero de paginas en tf del proyecto a cambiado a %.', proyectoRow.n_paginastf;
